...
const EVAL_FALLBACK = {
  qcm: {
    CM1: [
      {q:"Un objet technique est…", options:["fabriqué par l'être humain","trouvé dans la nature","toujours électrique"], bonne:0, pts:1},
      {q:"Quelle pièce produit la lumière dans une lampe torche ?", options:["L'ampoule","La pile","Le boîtier"], bonne:0, pts:1}
    ],
    CM2: [
      {q:"Quelle phrase décrit une fonction technique ?", options:["L'interrupteur ouvre et ferme le circuit.","Le vélo permet de se déplacer.","Le cartable est à la mode."], bonne:0, pts:1},
      {q:"Pourquoi les fils électriques sont-ils en cuivre ?", options:["Le cuivre est conducteur","Le cuivre est transparent","Le cuivre est isolant"], bonne:0, pts:1}
    ]
  },
  questions_fermees: {
    CM1: [{q:"Vrai ou faux : chaque pièce d'un objet a un rôle.", type:"vf", bonne:"vrai", pts:1}],
    CM2: [{q:"Vrai ou faux : les matières plastiques sont en général isolantes.", type:"vf", bonne:"vrai", pts:1}]
  },
  etudes_documents: {CM1:[], CM2:[]},
  fiches_preparatoires: [
    {salle:1, titre:"À quoi sert un objet ?", objectifs:["Relier un objet à un besoin"],
     vocabulaire:[{mot:"fonction d'usage", def:"ce à quoi sert l'objet"}],
     contexte:"L'entrée de l'atelier : cinq objets attendent leur étiquette."}
  ]
};
...
const MEDIA_DECORS = [
  ...
  {base:"salle1", nom:"Salle 1 · L'entrée de l'atelier"},
  {base:"salle2", nom:"Salle 2 · L'établi"},
  {base:"salle3", nom:"Salle 3 · La matériauthèque"},
  {base:"salle4", nom:"Salle 4 · La salle des machines"},
  {base:"salle5", nom:"Salle 5 · Le coin montage"},
  {base:"final",  nom:"Cinématique de fin"}
];
const MEDIA_PERSOS = [
  {base:"zoe",      nom:"Zoé (apprentie ingénieure)"},
  {base:"awa",      nom:"Awa (ouvrière de l'atelier)"},
  {base:"marcel",   nom:"Monsieur Marcel (réparateur de vélos)"},
  {base:"eleonore", nom:"Éléonore Marchand (l'inventrice)"}
];
  ...
  corps.innerHTML = `
    ...
    <div class="reglages-group">
      ...
      <div style="background:#fff8e6;border:1px solid var(--parchemin-ombre);border-radius:10px;padding:12px;margin-top:10px;font-size:.85rem">
        ...
        <code>assets/images/personnages/</code> : <code>zoe</code>, <code>awa</code>,
        <code>marcel</code>, <code>eleonore</code> (+ une variante <code>&lt;nom&gt;-parle.mp4</code>
...
function sauverReglages(){
  ...
  try{ localStorage.setItem("escape_reglages_objets_techniques", JSON.stringify(R)); }catch(e){}
  appliquerReglages();
  majHUD();
...
async function afficherCredits(){
  ...
  if(!M || !M.photos){
    zone.innerHTML = "<p style='opacity:.7'>Aucune photographie installée : le jeu utilise ses décors dessinés. " +
      "Voir <code>assets/README.md</code>.</p>";
    return;
  }
  ...
  zone.innerHTML = `
    ...
    personnages fictifs. Schémas des énigmes et des leçons : dessinés pour ce jeu.</p>`;
}
