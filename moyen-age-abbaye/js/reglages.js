...
const EVAL_FALLBACK = {
  qcm: {
    CM1: [
      {q:"Clovis est le roi…", options:["des Romains","des Francs","des Gaulois"], bonne:1, pts:1},
      {q:"Qui est couronné empereur en l'an 800 ?", options:["Clovis","Charlemagne","Suger"], bonne:1, pts:1}
    ],
    CM2: [
      {q:"Le Moyen Âge va de…", options:["476 à 1492","800 à 1789","52 av. J.-C. à 476"], bonne:0, pts:1},
      {q:"Quel élément gothique renvoie la poussée de la voûte vers l'extérieur ?", options:["Le contrefort","L'arc-boutant","La voûte en berceau"], bonne:1, pts:1}
    ]
  },
  questions_fermees: {
    CM1: [
      {q:"Vrai ou faux : au Moyen Âge, les livres sont recopiés à la main.", type:"vf", bonne:"vrai", pts:1},
      {q:"Comment s'appelle la salle où les moines copient les livres ?", type:"court", rep:"Le scriptorium", pts:1}
    ],
    CM2: [
      {q:"Vrai ou faux : Charlemagne a créé l'école pour tous les enfants.", type:"vf", bonne:"faux", pts:1},
      {q:"Donne la date exacte du couronnement de Charlemagne.", type:"court", rep:"Le 25 décembre 800", pts:1}
    ]
  },
  etudes_documents: { CM1: [], CM2: [] },
  fiches_preparatoires: [
    {salle:1, titre:"Après l'Empire romain : Clovis, roi des Francs", objectifs:["Savoir qui est Clovis"],
     vocabulaire:[{mot:"Francs", def:"Peuple installé au nord de la Gaule"}],
     contexte:"Clovis, roi des Francs, est baptisé à Reims vers l'an 500."}
  ]
};
...
const MEDIA_DECORS = [
  ...
  {base:"salle1", nom:"Salle 1 · Reims, le baptême de Clovis"},
  {base:"salle2", nom:"Salle 2 · Aix-la-Chapelle, la cour de Charlemagne"},
  {base:"salle3", nom:"Salle 3 · Le scriptorium de l'abbaye"},
  {base:"salle4", nom:"Salle 4 · L'hôtel-Dieu"},
  {base:"salle5", nom:"Salle 5 · Le chantier : église romane et cathédrale gothique"},
  {base:"final",  nom:"Cinématique de fin"}
];
const MEDIA_PERSOS = [
  {base:"anselme", nom:"Frère Anselme (moine copiste)"},
  {base:"aude",    nom:"Aude (élève de l'école du palais)"},
  {base:"alix",    nom:"Mère Alix (abbesse de l'hôtel-Dieu)"},
  {base:"garin",   nom:"Garin (apprenti tailleur de pierre)"}
];
  ...
  corps.innerHTML = `
    ...
    <div class="reglages-group">
      ...
      <div style="background:#fff8e6;border:1px solid var(--parchemin-ombre);border-radius:10px;padding:12px;margin-top:10px;font-size:.85rem">
        ...
        <code>assets/images/personnages/</code> : <code>anselme</code>, <code>aude</code>,
        <code>alix</code>, <code>garin</code> (+ une variante <code>&lt;nom&gt;-parle.mp4</code>
...
function sauverReglages(){
  ...
  try{ localStorage.setItem("escape_reglages_moyenage", JSON.stringify(R)); }catch(e){}
  appliquerReglages();
  majHUD();
