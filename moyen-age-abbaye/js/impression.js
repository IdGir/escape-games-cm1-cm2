...
function imprimerBilan(){
  ...
  const mention = part>=0.90 ? "🏆 Maître copiste"
  ...
  zone.innerHTML = `
    ...
    <div class="impr-titre-principal">Bilan — « Le Manuscrit de l'abbaye »</div>
    <div class="impr-soustitre">Escape game d'histoire · le Moyen Âge · cycle 3</div>
    ...
    <table style="width:100%;border-collapse:collapse;margin:14px 0;font-size:10pt">
      ...
      <tr><td style="padding:6px;border:1px solid #ccc">📜 Pages refaites</td><td style="padding:6px;border:1px solid #ccc"><b>${ETAT.motsCles.length} / 5</b> ${ETAT.motsCles.length?("("+ETAT.motsCles.join(" · ")+")"):""}</td></tr>
  ...
  return `<div class="impr-entete">
    <div class="gauche">
      ...
      <div>
        ...
        <div>Le Manuscrit de l'abbaye · le Moyen Âge · cycle 3</div>
      </div>
    </div>
    <div class="droite">
...
function piedPageFiche(page, total, section){
  return `<div class="impr-pied">
    <div>${section} · « Le Manuscrit de l'abbaye » · ${ETAT.niveau||""}</div>
...
function qcmHTML(niveau, corrige){
  ...
  let html = `
    ...
    <div class="impr-bandeau-tricolore"></div>
    <div class="impr-titre-principal">QCM — Le Moyen Âge</div>
...
function etudesDocsHTML(niveau, corrige){
  ...
  let html = `
    ...
    <div class="impr-titre-principal">Étude de documents</div>
      <div class="impr-soustitre">Niveau ${niveau} · le Moyen Âge</div>
    ${!corrige?cartoucheEleve(ETAT.equipe, niveau):""}
    <div class="${corrige?"impr-corrige":""}">
  `;
  docs.forEach((d, i)=>{
