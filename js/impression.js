...
function imprimerBilan(){
  ...
  let mention = ETAT.score>=78?"🏆 Maître de la Révolution"
    : ETAT.score>=64?"🥈 Patriote éclairé"
    : ETAT.score>=50?"🥉 Bon citoyen"
    : "📜 Apprenti motivé";
  zone.innerHTML = `
    ${enteteFiche("Bilan de partie", "Élève", ETAT.equipe, false)}
    <div class="impr-bandeau-tricolore"></div>
    <div class="impr-titre-principal">Bilan — « Le Secret de la Déclaration »</div>
    <div class="impr-soustitre">Escape game sur la Révolution française · 1789</div>
    ${cartoucheEleve(ETAT.equipe, ETAT.niveau)}
    <div style="text-align:center;margin:20px 0">
      <div style="font-size:2.6rem;color:#1d3a8a;font-weight:bold">${ETAT.score} <span style="font-size:1rem;color:#888">/ ${scoreMax} points</span></div>
    ...
    <table style="width:100%;border-collapse:collapse;margin:14px 0;font-size:10pt">
      ...
      <tr><td style="padding:6px;border:1px solid #ccc">📜 Fragments récupérés</td><td style="padding:6px;border:1px solid #ccc"><b>${ETAT.fragments.length} / 4</b> ${ETAT.fragments.length?("("+ETAT.fragments.join(", ")+")"):""}</td></tr>
  ...
  return `<div class="impr-entete">
    <div class="gauche">
      ...
      <div>
        ...
        <div>Le Secret de la Déclaration · Révolution française · 1789</div>
      </div>
    </div>
    <div class="droite">
...
function piedPageFiche(page, total, section){
  return `<div class="impr-pied">
    <div>${section} · « Le Secret de la Déclaration » · ${ETAT.niveau||""}</div>
...
function qcmHTML(niveau, corrige){
  ...
  let html = `
    ...
    <div class="impr-bandeau-tricolore"></div>
    <div class="impr-titre-principal">QCM — La Révolution française</div>
...
function etudesDocsHTML(niveau, corrige){
  ...
  let html = `
    ...
    <div class="impr-titre-principal">Étude de documents</div>
      <div class="impr-soustitre">Niveau ${niveau} · Révolution française</div>
    ${!corrige?cartoucheEleve(ETAT.equipe, niveau):""}
    <div class="${corrige?"impr-corrige":""}">
  `;
  docs.forEach((d, i)=>{
