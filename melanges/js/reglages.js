/* ============================================================
   ...
   - Sons et voix
   - Durée de la séance, leçons autorisées ou non
...
const EVAL_FALLBACK = {
  qcm: {
    CM1: [
      {q:"Quel instrument mesure une masse ?", options:["un thermomètre","une balance","une règle"], bonne:1, pts:1},
      {q:"1 kg = …", options:["100 g","1 000 g","10 g"], bonne:1, pts:1},
      {q:"L'eau salée est un mélange…", options:["homogène","hétérogène"], bonne:0, pts:1},
      {q:"Pour séparer la limaille de fer du sable, on utilise…", options:["un tamis","un aimant","un filtre"], bonne:1, pts:1}
    ],
    CM2: [
      {q:"0,25 kg = …", options:["25 g","250 g","2 500 g"], bonne:1, pts:1},
      {q:"250 g d'eau + 20 g de sel, après dissolution :", options:["250 g","270 g","230 g"], bonne:1, pts:1},
      {q:"L'eau du robinet est…", options:["de l'eau pure","un mélange homogène","un mélange hétérogène"], bonne:1, pts:1},
      {q:"Pour récupérer le sel dissous, on…", options:["filtre","fait évaporer l'eau","tamise"], bonne:1, pts:1}
    ]
  },
  questions_fermees: {
    CM1: [
      {q:"Vrai ou faux : quand on dissout du sucre dans l'eau, la masse ne change pas.", type:"vf", bonne:"vrai", pts:1},
      {q:"Quelle méthode permet de récupérer le sel de l'eau salée ?", type:"court", rep:"L'évaporation", pts:1}
    ],
    CM2: [
      {q:"Vrai ou faux : on peut retirer le sel de l'eau salée par filtration.", type:"vf", bonne:"faux", pts:1},
      {q:"Convertis : 1,2 kg = ______ g.", type:"court", rep:"1 200", pts:1}
    ]
  },
  etudes_documents: {CM1:[], CM2:[]},
  fiches_preparatoires: [
    {salle:1, titre:"Mesurer et comparer des masses", objectifs:["Comparer et mesurer des masses"],
     vocabulaire:[{mot:"Masse", def:"Ce que mesure une balance, en grammes ou en kilogrammes"}],
     contexte:"Dans la salle des balances du laboratoire de Madame Mélange."}
  ]
};
...
const MEDIA_DECORS = [
  ...
  {base:"salle1", nom:"Salle 1 · La salle des balances"},
  {base:"salle2", nom:"Salle 2 · La cuisine d'essai"},
  {base:"salle3", nom:"Salle 3 · La salle des fioles"},
  {base:"salle4", nom:"Salle 4 · L'atelier de tri"},
  {base:"salle5", nom:"Salle 5 · La saline"},
  {base:"final",  nom:"Cinématique de fin"}
];
const MEDIA_PERSOS = [
  {base:"lila",    nom:"Lila (apprentie chimiste)"},
  {base:"marius",  nom:"Marius (cuisinier)"},
  {base:"nadia",   nom:"Nadia (laborantine)"},
  {base:"yann",    nom:"Yann (paludier)"},
  {base:"melange", nom:"Madame Mélange (chimiste)"}
];
  ...
  corps.innerHTML = `
    ...
    <div class="reglages-group">
      ...
      <div style="background:#fff8e6;border:1px solid var(--parchemin-ombre);border-radius:10px;padding:12px;margin-top:10px;font-size:.85rem">
        ...
        <code>assets/images/personnages/</code> : <code>lila</code>, <code>marius</code>,
        <code>nadia</code>, <code>yann</code>, <code>melange</code> (+ une variante <code>&lt;nom&gt;-parle.mp4</code>
    ...
    <div class="reglages-group">
      ...
      <div class="reglage-ligne">
        <div class="libelle"><b>Leçons 📚</b><br><span style="font-size:.8rem;opacity:.7">Les cinq leçons du laboratoire, consultables pendant la partie</span></div>
...
function sauverReglages(){
  ...
  try{ localStorage.setItem("escape_reglages_melanges", JSON.stringify(R)); }catch(e){}
  appliquerReglages();
  majHUD();
