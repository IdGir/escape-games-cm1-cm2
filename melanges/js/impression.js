...
function imprimerBilan(){
  ...
  const max = (typeof scoreMax === "function") ? scoreMax() : 125;
  const part = max ? ETAT.score/max : 0;
  const mention = part>=0.90 ? "Grand chimiste"
                : part>=0.75 ? "Chimiste confirmé"
                : part>=0.55 ? "Apprenti chimiste"
                : "Apprenti motivé";
  const total = (typeof nbEnigmesTotal === "function") ? nbEnigmesTotal() : 0;
  zone.innerHTML = `
    ${enteteFiche("Bilan de partie", "Élève", ETAT.equipe, false)}
    <div class="impr-bandeau-tricolore"></div>
    <div class="impr-titre-principal">Bilan — « Le Laboratoire de Madame Mélange »</div>
    <div class="impr-soustitre">Escape game de sciences · masses, mélanges et séparation · cycle 3</div>
    ${cartoucheEleve(ETAT.equipe, ETAT.niveau)}
    <div style="text-align:center;margin:20px 0">
      <div style="font-size:2.6rem;color:#1d3a8a;font-weight:bold">${ETAT.score} <span style="font-size:1rem;color:#888">/ ${max} points</span></div>
      <div style="font-size:1.2rem;color:#b22222;font-weight:bold">${mention}</div>
    </div>
    <table style="width:100%;border-collapse:collapse;margin:14px 0;font-size:10pt">
      ...
      <tr><td style="padding:6px;border:1px solid #ccc">🧩 Énigmes résolues</td><td style="padding:6px;border:1px solid #ccc"><b>${ETAT.enigmesReussies} / ${total}</b></td></tr>
      <tr><td style="padding:6px;border:1px solid #ccc">🧪 Opérations du protocole</td><td style="padding:6px;border:1px solid #ccc"><b>${ETAT.motsCles.length} / 5</b> ${ETAT.motsCles.length?("("+ETAT.motsCles.join(" · ")+")"):""}</td></tr>
      <tr><td style="padding:6px;border:1px solid #ccc">💡 Indices utilisés</td><td style="padding:6px;border:1px solid #ccc"><b>${ETAT.indicesTotal}</b></td></tr>
...
async function imprimerFiches(type){
  const EVAL_DATA = await chargerEvaluations();   // source unique, quel que soit l'ordre des scripts
  await chargerLecons();
  DONNEES_EVAL = EVAL_DATA;
  const zone = preparerZoneImpression();
  ...
  return `<div class="impr-entete">
    <div class="gauche">
      ...
      <div>
        ...
        <div>Le Laboratoire de Madame Mélange · sciences · cycle 3</div>
      </div>
    </div>
    <div class="droite">
...
function piedPageFiche(page, total, section){
  return `<div class="impr-pied">
    <div>${section} · « Le Laboratoire de Madame Mélange » · ${ETAT.niveau||""}</div>
    <div>Page ${page} / ${total}</div>
  </div>`;
}

/* Copie locale des évaluations, remplie par imprimerFiches(). */
var DONNEES_EVAL = null;

/* ============================================================
   FICHES PRÉPARATOIRES
   ============================================================ */
function fichesPreparatoiresHTML(){
  const fiches = (DONNEES_EVAL && DONNEES_EVAL.fiches_preparatoires) || [];
  let html = "";
  fiches.forEach((f, i)=>{
    html += (i>0?sautPage():"") + `
      ${enteteFiche("Fiche préparatoire — Salle "+f.salle, "Élève", ETAT.equipe, false)}
      <div class="impr-bandeau-tricolore"></div>
      ...
      <div class="impr-fiche-prepa">
        ...
        <div class="impr-note-enseignant">💡 À lire avant de commencer la salle ${f.salle} du jeu.</div>
      </div>
...
function qcmHTML(niveau, corrige){
  const questions = (DONNEES_EVAL && DONNEES_EVAL.qcm && DONNEES_EVAL.qcm[niveau]) || [];
  const total = questions.reduce((s,q)=>s+q.pts,0);
  let html = `
    ${enteteFiche("QCM — "+niveau, corrige?"Corrigé":"Évaluation", ETAT.equipe, corrige)}
    <div class="impr-bandeau-tricolore"></div>
    <div class="impr-titre-principal">QCM — Masses, mélanges et séparation</div>
...
function fermeesHTML(niveau, corrige){
  const questions = (DONNEES_EVAL && DONNEES_EVAL.questions_fermees && DONNEES_EVAL.questions_fermees[niveau]) || [];
  const total = questions.reduce((s,q)=>s+q.pts,0);
  let html = `
...
function etudesDocsHTML(niveau, corrige){
  const docs = (DONNEES_EVAL && DONNEES_EVAL.etudes_documents && DONNEES_EVAL.etudes_documents[niveau]) || [];
  let html = `
    ...
    <div class="impr-titre-principal">Étude de documents</div>
      <div class="impr-soustitre">Niveau ${niveau} · masses, mélanges et séparation</div>
    ${!corrige?cartoucheEleve(ETAT.equipe, niveau):""}
    <div class="${corrige?"impr-corrige":""}">
  `;
  docs.forEach((d, i)=>{
...
window.preparerZoneImpression = preparerZoneImpression;
window.enteteFiche            = enteteFiche;
window.cartoucheEleve         = cartoucheEleve;
window.piedPageFiche          = piedPageFiche;
window.sautPage               = sautPage;
