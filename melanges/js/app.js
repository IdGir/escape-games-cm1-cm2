/* ============================================================
   APP — Moteur principal du jeu « Le Laboratoire de Madame Mélange »
   Coordonne : écrans, salles, décors, dialogues, énigmes,
               opérations du testament, score, minuteur, leçons, réglages
   ------------------------------------------------------------
...
const ETAT = {
  ...
  score: 0,
  motsCles: [],              // opérations du testament déjà retrouvées
  badges: {vitesse:false, oeil:false, main:false, heritier:false},
  debut: null,
  msEcoules: 0,
  enPause: false,
  tempsParSalle: {},
...
const CLE_SAUVEGARDE = "escape_melanges_v1";
...
async function chargerDonnees(){
  ...
  try{
    const r = localStorage.getItem("escape_reglages_melanges");
...
const DONNEES_FALLBACK = {
  personnages: {
    lila:{nom:"Lila"}, marius:{nom:"Marius"}, nadia:{nom:"Nadia"},
    yann:{nom:"Yann"}, melange:{nom:"Madame Mélange"}
  },
  protocole:["PESER","OBSERVER","AIMANTER","ÉVAPORER","COMPARER"],
  salles:[
    {num:1, titre:"La salle des balances", decor:"balances", motCle:"PESER",
     lieu:"La salle des balances", description:"Des balances de toutes les tailles.",
     dialogue_intro:{perso:"lila", nom:"Lila", texte:"Avant de séparer, on pèse."},
     dialogue_reussite:{perso:"lila", nom:"Lila", texte:"Première opération : PESER."}},
    {num:2, titre:"La cuisine d'essai", decor:"cuisine", motCle:"COMPARER",
     lieu:"La cuisine d'essai", description:"Un sucrier, une tasse, une balance.",
     dialogue_intro:{perso:"marius", nom:"Marius", texte:"Mon sucre a-t-il disparu ?"},
     dialogue_reussite:{perso:"marius", nom:"Marius", texte:"Deuxième opération : COMPARER."}},
    {num:3, titre:"La salle des fioles", decor:"fioles", motCle:"OBSERVER",
     lieu:"La salle des fioles", description:"Des fioles de toutes les couleurs.",
     dialogue_intro:{perso:"lila", nom:"Lila", texte:"Homogène ou hétérogène ?"},
     dialogue_reussite:{perso:"lila", nom:"Lila", texte:"Troisième opération : OBSERVER."}},
    {num:4, titre:"L'atelier de tri", decor:"atelier", motCle:"AIMANTER",
     lieu:"L'atelier de tri", description:"Tamis, aimant, bassine d'eau.",
     dialogue_intro:{perso:"nadia", nom:"Nadia", texte:"Chaque méthode utilise une différence."},
     dialogue_reussite:{perso:"nadia", nom:"Nadia", texte:"Quatrième opération : AIMANTER."}},
    {num:5, titre:"La saline", decor:"saline", motCle:"ÉVAPORER",
     lieu:"La saline", description:"Filtres, béchers et marais salants.",
     dialogue_intro:{perso:"yann", nom:"Yann", texte:"Le sel dissous traverse le filtre."},
     dialogue_fin:{perso:"melange", nom:"Madame Mélange", texte:"Rien ne s'est perdu : la matière ne disparaît pas."}}
  ]
};

const ENIGMES_FALLBACK = { salles:[
  {num:1, enigmes:[{id:"secours-1", titre:"Peser", type:"qcm",
    consigne:"Choisis la bonne réponse.", commun:{questions:[{q:"Quel instrument mesure une masse ?",
      options:["une balance","une règle","un thermomètre"], bonne:0}]}}]},
...
/** Les cinq opérations du testament, telles qu'elles sont à l'instant t
    (dans l'ordre des salles : l'ordre du protocole se découvre à la fin). */
function serruresHTML(){
  return `<div class="coffre-serrures" aria-label="Opérations du testament retrouvées">${DONNEES.salles.map((s,i)=>{
    ...
    return `<div class="serrure ${ouverte?"ouverte":""}">
      ...
      <span class="mot">${ouverte?s.motCle:"Salle "+s.num+" : ?"}</span>
    </div>`;
  }).join("")}</div>`;
}
/** Le testament complet, dans l'ordre du protocole. */
function protocoleHTML(){
  const p = (DONNEES.protocole && DONNEES.protocole.length) ? DONNEES.protocole : DONNEES.salles.map(s=>s.motCle);
  return `<ol class="protocole-final">${p.map(m=>`<li><b>${m}</b></li>`).join("")}</ol>`;
}
...
document.addEventListener("DOMContentLoaded", async ()=>{
  ...
  try{
    const vStockee = localStorage.getItem("escape_app_version_melanges");
    ...
    localStorage.setItem("escape_app_version_melanges", VERSION_APP);
  }catch(e){}
  ...
  document.getElementById("btn-demarrer").addEventListener("click", ()=>{
    ...
    lancerCine("intro", "Le laboratoire de Madame Mélange", ()=>entrerDansLeJeu(false));
  });
...
function afficherSalle(n){
  ...
  c.innerHTML = `
    ${barreProgression()}
    <h2>🧪 Salle ${salle.num}/${NB_SALLES} — ${salle.titre}</h2>
...
/* ---- Salle bouclée : une opération du testament est retrouvée ---- */
function validerSalle(n){
  ...
  zone.innerHTML = `
    <div class="mot-cle">
      <div class="lib">Opération retrouvée dans la salle ${n}</div>
  ...
  if(n === NB_SALLES){
    setTimeout(()=>lancerCine("final", "Le testament est complet", finDuJeu), 1600);
    return;
  }
...
function attribuerBadges(n, duree){
  ...
  if(n === 3 && ETAT.indicesSalle === 0) ETAT.badges.oeil = true;       // salle des fioles sans indice
  if(n === 4 && ETAT.indicesSalle === 0) ETAT.badges.main = true;       // atelier de tri sans indice
  if(n === NB_SALLES && ETAT.indicesTotal <= 3) ETAT.badges.heritier = true;
}
...
function finDuJeu(){
  ...
  c.innerHTML = `
    <h2>🧪 Le testament de Madame Mélange est complet !</h2>
    <div id="fin-dialogue"></div>
    ${serruresHTML()}
    <div class="article-secret testament-final">
      <div class="numero">LE PROTOCOLE DE MADAME MÉLANGE</div>
      <div class="texte">Pour séparer mon dernier bocal (500 g d'eau, 30 g de sel dissous, 20 g de limaille de fer) :</div>
      ${protocoleHTML()}
      <div class="texte">20 g de fer + 30 g de sel = 50 g : exactement ce qui avait été mis. La matière ne disparaît pas.</div>
      <div style="font-size:.8rem;opacity:.8">Rappel : au laboratoire, on observe et on mesure. On ne goûte jamais, on ne sent jamais un produit.</div>
    </div>
    <div class="grille-badges">
      ${badgeHTML("vitesse","⏱️","Équipe rapide","une salle en moins de 6 minutes")}
      ${badgeHTML("oeil","🔍","Œil de lynx","salle des fioles sans aucun indice")}
      ${badgeHTML("main","🧲","Main sûre","atelier de tri sans aucun indice")}
      ${badgeHTML("heritier","⚗️","Héritier de Madame Mélange","3 indices au maximum sur toute la partie")}
    </div>
...
const QUIZZ_FALLBACK = {
  CM1:[
    {q:"Sur une balance à plateaux, les deux plateaux sont à la même hauteur. Les masses sont…", options:["différentes","égales","nulles"], bonne:1},
    {q:"On dissout 10 g de sel dans 200 g d'eau. Le mélange pèse…", options:["200 g","210 g","190 g"], bonne:1},
    {q:"L'eau salée est…", options:["un mélange homogène","un mélange hétérogène","de l'eau pure"], bonne:0},
    {q:"Pour séparer la limaille de fer du sable, on utilise…", options:["un tamis","un aimant","un filtre"], bonne:1},
    {q:"Pour récupérer le sel dissous dans l'eau, on…", options:["filtre l'eau","fait évaporer l'eau","tamise l'eau"], bonne:1}
  ],
  CM2:[
    {q:"Combien de grammes dans 1,5 kg ?", options:["150 g","1 500 g","15 000 g"], bonne:1},
    {q:"Un bol vide pèse 150 g ; plein de lait, 400 g. La masse du lait est…", options:["550 g","250 g","400 g"], bonne:1},
    {q:"Lequel de ces mélanges est hétérogène ?", options:["l'air","le sirop à l'eau","la vinaigrette"], bonne:2},
    {q:"Pourquoi ne peut-on pas retirer le sel de l'eau salée par filtration ?", options:["Le sel dissous traverse le filtre","Le sel bouche le filtre","Le sel est plus lourd que l'eau"], bonne:0},
    {q:"Dans le protocole de Madame Mélange, quelle opération vient juste avant ÉVAPORER ?", options:["PESER","AIMANTER","COMPARER"], bonne:1}
  ]
};
...
function construireQuizz(){
  ...
  document.getElementById("btn-voir-score").onclick = ()=>{
    ...
    fb.innerHTML = `Tu as <b>${score}/${QUIZZ.length}</b> bonnes réponses (+${score*PTS_QUIZ} points). ${
      score===5?"Parfait : tu es un vrai chimiste !" :
      score>=4?"Très bien ! Encore un petit effort." :
      score>=3?"Pas mal. Relis la leçon 📚 sur les mélanges homogènes et hétérogènes." :
               "Reprends les leçons 📚 : la masse se conserve, et le sel dissous traverse le filtre."}`;
    afficherRecap();
    sauvegarder();
  };
}
...
function afficherRecap(){
  ...
  const mention = part>=0.9 ? "Grand chimiste"
                : part>=0.75 ? "Chimiste confirmé"
                : part>=0.55 ? "Apprenti chimiste"
                : "Apprenti motivé";
  ...
  recap.innerHTML = `
    <div class="article-secret">
      ...
      <div>🧪 Opérations : <b>${ETAT.motsCles.length}/${NB_SALLES}</b></div>
...
window.protocoleHTML = protocoleHTML;
