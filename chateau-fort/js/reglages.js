...
const EVAL_FALLBACK = {
  qcm: {
    CM1: [
      {q:"Les premiers châteaux forts étaient en…", options:["pierre","terre et bois","brique"], bonne:1, pts:1},
      {q:"Le donjon est…", options:["une prison","la tour maîtresse du seigneur","un fossé"], bonne:1, pts:1}
    ],
    CM2: [
      {q:"À qui est versée la dîme ?", options:["à l'Église","au seigneur","au roi"], bonne:0, pts:1},
      {q:"Que sont les banalités ?", options:["des jours de fête","l'obligation d'utiliser le moulin, le four et le pressoir du seigneur","une part des récoltes pour l'Église"], bonne:1, pts:1}
    ]
  },
  questions_fermees: {
    CM1: [
      {q:"Vrai ou faux : le pont-levis se relève pour fermer l'entrée.", type:"vf", bonne:"vrai", pts:1},
      {q:"Comment s'appelle le travail gratuit dû au seigneur ?", type:"court", rep:"La corvée", pts:1}
    ],
    CM2: [
      {q:"Vrai ou faux : tous les châteaux forts datent du XIIIe siècle.", type:"vf", bonne:"faux", pts:1},
      {q:"Donne les trois fonctions d'un château fort.", type:"court", rep:"Protection, lieu de vie du seigneur, symbole de puissance", pts:1}
    ]
  },
  etudes_documents: { CM1: [], CM2: [] },
  fiches_preparatoires: [
    {salle:1, titre:"La motte et la palissade", objectifs:["Dire pourquoi on construit un château fort"],
     vocabulaire:[{mot:"motte", def:"Butte de terre qui porte une tour de bois"}],
     contexte:"Vers le Xe siècle, les premiers châteaux sont en terre et en bois."}
  ]
};
...
const MEDIA_DECORS = [
  ...
  {base:"salle1", nom:"Salle 1 · La motte et la palissade"},
  {base:"salle2", nom:"Salle 2 · Les remparts"},
  {base:"salle3", nom:"Salle 3 · La grande salle"},
  {base:"salle4", nom:"Salle 4 · Le village et les champs"},
  {base:"salle5", nom:"Salle 5 · Le moulin du seigneur"},
  {base:"final",  nom:"Cinématique de fin"}
];
const MEDIA_PERSOS = [
  {base:"colin",    nom:"Colin (page du seigneur)"},
  {base:"josselin", nom:"Maître Josselin (maître maçon)"},
  {base:"alienor",  nom:"Dame Aliénor (dame du château)"},
  {base:"mahaut",   nom:"Mahaut (jeune paysanne)"},
  {base:"perrine",  nom:"Perrine (meunière)"}
];
  ...
  corps.innerHTML = `
    ...
    <div class="reglages-group">
      ...
      <div style="background:#fff8e6;border:1px solid var(--parchemin-ombre);border-radius:10px;padding:12px;margin-top:10px;font-size:.85rem">
        ...
        <code>assets/images/personnages/</code> : <code>colin</code>, <code>josselin</code>,
        <code>alienor</code>, <code>mahaut</code>, <code>perrine</code> (+ une variante <code>&lt;nom&gt;-parle.mp4</code>
    ...
    <div class="reglages-group">
      ...
      <div class="reglage-ligne">
        <div class="libelle"><b>Leçons 📚</b><br><span style="font-size:.8rem;opacity:.7">Les cinq leçons du jeu, consultables pendant la partie (bouton du haut et bouton sous chaque énigme)</span></div>
...
function sauverReglages(){
  ...
  try{ localStorage.setItem("escape_reglages_chateau_fort", JSON.stringify(R)); }catch(e){}
  appliquerReglages();
  majHUD();
