...
function imprimerBilan(){
  ...
  const mention = part>=0.90 ? "🏆 Chef de station"
  ...
  zone.innerHTML = `
    ...
    <div class="impr-titre-principal">Bilan — « La Station météo disparue »</div>
    <div class="impr-soustitre">Escape game de sciences · mesures météorologiques · cycle 3</div>
    ...
    <table style="width:100%;border-collapse:collapse;margin:14px 0;font-size:10pt">
      ...
      <tr><td style="padding:6px;border:1px solid #ccc">🛠️ Modules remis en service</td><td style="padding:6px;border:1px solid #ccc"><b>${ETAT.motsCles.length} / 5</b> ${ETAT.motsCles.length?("("+ETAT.motsCles.join(" · ")+")"):""}</td></tr>
  ...
  return `<div class="impr-entete">
    <div class="gauche">
      ...
      <div>
        ...
        <div>La Station météo disparue · mesures météorologiques · cycle 3</div>
      </div>
    </div>
    <div class="droite">
...
function piedPageFiche(page, total, section){
  return `<div class="impr-pied">
    <div>${section} · « La Station météo disparue » · ${ETAT.niveau||""}</div>
...
function fichesPreparatoiresHTML(){
  ...
  fiches.forEach((f, i)=>{
    html += (i>0?sautPage():"") + `
      ${enteteFiche("Fiche préparatoire — Module "+f.salle, "Élève", ETAT.equipe, false)}
      <div class="impr-bandeau-tricolore"></div>
...
function qcmHTML(niveau, corrige){
  ...
  let html = `
    ...
    <div class="impr-bandeau-tricolore"></div>
    <div class="impr-titre-principal">QCM — Mesurer le temps qu'il fait</div>
...
function etudesDocsHTML(niveau, corrige){
  ...
  let html = `
    ...
    <div class="impr-titre-principal">Étude de documents</div>
      <div class="impr-soustitre">Niveau ${niveau} · mesures météorologiques</div>
    ${!corrige?cartoucheEleve(ETAT.equipe, niveau):""}
    <div class="${corrige?"impr-corrige":""}">
  `;
  docs.forEach((d, i)=>{
