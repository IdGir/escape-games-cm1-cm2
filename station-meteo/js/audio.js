...
function ambiance(nom){
  ...
  switch(nom){
    /* ---- Les cinq lieux de « La Station météo disparue » ---- */
    case "abri":       ambianceJardins(noeuds); break;   // la cour de l'école, oiseaux
    case "mat":        ambianceVent(noeuds); break;      // souffle du vent sur le mât
    case "pluvio":     ambiancePluie(noeuds); break;     // le jardin sous la pluie
    case "bureau":     ambianceAccueil(noeuds); break;   // le bureau des relevés
    case "studio":     ambianceAccueil(noeuds); break;   // la salle de prévision
...
/* ---- Module 2 : le vent dans le mât de l'anémomètre ----
   Bruit filtré en passe-bas, modulé lentement : un souffle qui
   enfle et retombe, sans rien d'inquiétant. */
function ambianceVent(n){
  const ctx = AUDIO.ctx;
  const s = sourceBruit();
  const lp = ctx.createBiquadFilter();
  ...
  s.start();
  n.sources.push(s);
  // Rafales lentes : le volume et la coupure respirent
  const lfo = ctx.createOscillator(), lfoG = ctx.createGain();
  lfo.frequency.value = 0.07; lfoG.gain.value = 0.11;
  lfo.connect(lfoG).connect(g.gain); lfo.start(); n.sources.push(lfo);
  const lfo2 = ctx.createOscillator(), lfo2G = ctx.createGain();
  lfo2.frequency.value = 0.045; lfo2G.gain.value = 180;
  lfo2.connect(lfo2G).connect(lp.frequency); lfo2.start(); n.sources.push(lfo2);
}

/* ---- Module 3 : la pluie sur le jardin ---- */
