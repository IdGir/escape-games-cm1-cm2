/* ============================================================
   ...
   - Sons et voix
   - Durée de la séance, leçons consultables
...
const EVAL_FALLBACK = {
  qcm: {
    CM1: [
      {q:"À quelle hauteur mesure-t-on la température de l'air sous abri ?", options:["Au ras du sol","À 1,50 m du sol","À 10 m du sol"], bonne:1, pts:1},
      {q:"Que mesure une girouette ?", options:["La direction d'où vient le vent","La vitesse du vent","La hauteur de pluie"], bonne:0, pts:1},
      {q:"Dans quelle unité lit-on un pluviomètre ?", options:["En litres","En millimètres","En degrés"], bonne:1, pts:1},
      {q:"1 mm de pluie, c'est…", options:["1 litre par mètre carré","1 litre pour toute la cour","1 goutte par mètre carré"], bonne:0, pts:1}
    ],
    CM2: [
      {q:"Pourquoi l'abri météo est-il blanc et ventilé ?", options:["Pour protéger le capteur du soleil et de la pluie","Pour le réchauffer","Pour le voir de loin"], bonne:0, pts:1},
      {q:"À quelle hauteur mesure-t-on le vent en terrain dégagé ?", options:["1,50 m","10 m","50 m"], bonne:1, pts:1},
      {q:"Un vent de nord-ouest…", options:["vient du nord-ouest","va vers le nord-ouest","tourne au nord-ouest"], bonne:0, pts:1},
      {q:"Sur quelle durée calcule-t-on une moyenne climatique ?", options:["Un mois","Un an","Trente ans"], bonne:2, pts:1}
    ]
  },
  questions_fermees: {
    CM1: [
      {q:"Vrai ou faux : on mesure la température de l'air en plein soleil.", type:"vf", bonne:"faux", pts:1},
      {q:"Quel instrument mesure la vitesse du vent ?", type:"court", rep:"L'anémomètre", pts:1}
    ],
    CM2: [
      {q:"Vrai ou faux : la girouette montre la direction vers laquelle souffle le vent.", type:"vf", bonne:"faux", pts:1},
      {q:"Il est tombé 3 mm, 0 mm puis 5 mm. Quel est le cumul ?", type:"court", rep:"8 mm", pts:1}
    ]
  },
  etudes_documents: {CM1:[], CM2:[]},
  fiches_preparatoires: [
    {salle:1, titre:"Mesurer la température", objectifs:["Lire un thermomètre"],
     vocabulaire:[{mot:"Abri météorologique", def:"Boîte blanche et ventilée qui protège le capteur du soleil et de la pluie"}],
     contexte:"Météo-France mesure la température de l'air sous abri, à 1,50 m du sol."}
  ]
};
...
const MEDIA_DECORS = [
  ...
  {base:"salle1", nom:"Module 1 · L'abri météo"},
  {base:"salle2", nom:"Module 2 · Le mât du vent"},
  {base:"salle3", nom:"Module 3 · Le pluviomètre"},
  {base:"salle4", nom:"Module 4 · Le tableau des relevés"},
  {base:"salle5", nom:"Module 5 · Le bulletin du jour"},
  {base:"final",  nom:"Cinématique de fin"}
];
const MEDIA_PERSOS = [
  {base:"vasseur", nom:"Madame Vasseur (prévisionniste)"},
  {base:"tiago",   nom:"Tiago (technicien de la station)"},
  {base:"lina",    nom:"Lina (responsable de la station de l'école)"},
  {base:"keita",   nom:"Capitaine Keïta (marin)"}
];
  ...
  corps.innerHTML = `
    ...
    <div class="reglages-group">
      ...
      <div style="background:#fff8e6;border:1px solid var(--parchemin-ombre);border-radius:10px;padding:12px;margin-top:10px;font-size:.85rem">
        ...
        <code>assets/images/personnages/</code> : <code>vasseur</code>, <code>tiago</code>,
        <code>lina</code>, <code>keita</code> (+ une variante <code>&lt;nom&gt;-parle.mp4</code>
    ...
    <div class="reglages-group">
      ...
      <div class="reglage-ligne">
        <div class="libelle"><b>Leçons 📚</b><br><span style="font-size:.8rem;opacity:.7">Les cinq leçons du jeu, consultables pendant la partie</span></div>
    ...
    <div class="reglages-group">
      ...
      <div style="display:flex;flex-direction:column;gap:8px">
        <button class="btn bleu" id="btn-imprimer-prepa">📋 Fiches préparatoires (5 modules)</button>
...
function sauverReglages(){
  ...
  try{ localStorage.setItem("escape_reglages_meteo", JSON.stringify(R)); }catch(e){}
  appliquerReglages();
  majHUD();
...
async function scannerMedias(){
  ...
  zone.innerHTML =
    ...
    + `<p style="font-size:.78rem;opacity:.7;margin-top:6px">
        ...
        Les leçons de ce jeu sont des textes rédigés : elles ne dépendent d'aucun fichier.
