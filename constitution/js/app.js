/* ============================================================
   APP — Moteur principal du jeu « Le Sceau de la République »
   Coordonne : écrans, salles, décors, dialogues, énigmes,
               serrures du coffre, score, minuteur, leçons, réglages
   ------------------------------------------------------------
...
const ETAT = {
  ...
  badges: {vitesse:false, juriste:false, citoyen:false, gardien:false},
  debut: null,
  msEcoules: 0,
  enPause: false,
  tempsParSalle: {},
  ...
  reglages: {
    ...
    sonVideo: false,
    cinematiques: true,
    // ---- Option enseignant : annonce du concours en fin de partie ----
    concoursActif: false,
  }
};
...
async function chargerDonnees(){
  ...
  CONCOURS = await chargerJSON("assets/data/concours.json");
  ...
  try{
    const r = localStorage.getItem("escape_reglages_constitution");
...
const DONNEES_FALLBACK = {
  personnages: {
    berthier:{nom:"Monsieur Berthier", emoji:"🗝️"},
    nour:{nom:"Nour", emoji:"🎒"},
    ferrand:{nom:"Madame Ferrand", emoji:"🎀"},
    sylla:{nom:"Maître Sylla", emoji:"⚖️"}
  },
  salles:[
    {num:1, titre:"La cour du Palais-Royal", decor:"cour", motCle:"RÈGLES",
     lieu:"🏛️ La cour du Palais-Royal — aujourd'hui",
     description:"Un coffre scellé le 4 octobre 1958 attend d'être ouvert.",
     dialogue_intro:{perso:"berthier", nom:"Monsieur Berthier", texte:"Bienvenue. Ce coffre porte cinq serrures. À vous de les ouvrir."},
     dialogue_reussite:{perso:"berthier", nom:"Monsieur Berthier", texte:"Première serrure ouverte."}},
    {num:2, titre:"La salle des Textes", decor:"archives", motCle:"TEXTES",
     lieu:"📜 La salle des Textes", description:"Quatre textes sous vitrine.",
     dialogue_intro:{perso:"sylla", nom:"Maître Sylla", texte:"Voici les textes sur lesquels nous travaillons."},
     dialogue_reussite:{perso:"sylla", nom:"Maître Sylla", texte:"Deuxième serrure ouverte."}},
    {num:3, titre:"L'hémicycle", decor:"hemicycle", motCle:"PEUPLE",
     lieu:"🏛️ L'hémicycle de l'Assemblée nationale", description:"Les bancs sont vides, le tableau de vote s'allume.",
     dialogue_intro:{perso:"ferrand", nom:"Madame Ferrand", texte:"Ici, le peuple parle par ses représentants."},
     dialogue_reussite:{perso:"ferrand", nom:"Madame Ferrand", texte:"Troisième serrure ouverte."}},
    {num:4, titre:"La navette parlementaire", decor:"senat", motCle:"LOI",
     lieu:"🏛️ De l'Assemblée nationale au Sénat", description:"Un texte fait l'aller-retour entre les deux palais.",
     dialogue_intro:{perso:"ferrand", nom:"Madame Ferrand", texte:"Suivons le chemin d'une loi."},
     dialogue_reussite:{perso:"ferrand", nom:"Madame Ferrand", texte:"Quatrième serrure ouverte."}},
    {num:5, titre:"La salle des séances", decor:"conseil", motCle:"GARDIEN",
     lieu:"⚖️ La salle des séances du Conseil constitutionnel", description:"Neuf sièges en fer à cheval.",
     dialogue_intro:{perso:"sylla", nom:"Maître Sylla", texte:"Dernière serrure. Montrez-moi ce que vous avez compris."},
     dialogue_fin:{perso:"berthier", nom:"Monsieur Berthier", texte:"Le coffre s'ouvre."}}
  ]
};

const ENIGMES_FALLBACK = { salles:[
  {num:1, enigmes:[{id:"secours-1", titre:"Qu'est-ce qu'une Constitution ?", type:"qcm",
    consigne:"Choisis la bonne réponse.", commun:{questions:[{q:"Une Constitution, c'est…",
      options:["l'ensemble des règles qui organisent la vie de tous les citoyens d'un pays",
               "une loi comme les autres","le règlement intérieur d'une école"], bonne:0}]}}]},
...
document.addEventListener("DOMContentLoaded", async ()=>{
  ...
  try{
    const vStockee = localStorage.getItem("escape_app_version_constitution");
    ...
    localStorage.setItem("escape_app_version_constitution", VERSION_APP);
  }catch(e){}
  ...
  document.getElementById("btn-demarrer").addEventListener("click", ()=>{
    ...
    lancerCine("intro", "Palais-Royal, Paris", ()=>entrerDansLeJeu(false));
  });
...
function afficherSalle(n){
  ...
  c.innerHTML = `
    ${barreProgression()}
    <h2>🗝️ Serrure ${salle.num}/${NB_SALLES} — ${salle.titre}</h2>
...
function validerSalle(n){
  ...
  zone.innerHTML = `
    <div class="mot-cle">
      <div class="lib">Serrure ${n} ouverte — mot-clé</div>
  ...
  if(n === NB_SALLES){
    setTimeout(()=>lancerCine("final", "Le coffre s'ouvre", finDuJeu), 1600);
    return;
  }
  ...
  function boutonSuivant(){
    ...
    b.className = "boutons";
    b.innerHTML = `<button class="btn grand vert" id="btn-salle-suivante">➡️ Serrure suivante</button>`;
    zone.appendChild(b);
...
function attribuerBadges(n, duree){
  ...
  if(n === 4 && ETAT.indicesSalle === 0) ETAT.badges.juriste = true;
  if(n === 3 && ETAT.indicesSalle === 0) ETAT.badges.citoyen = true;
...
function finDuJeu(){
  ...
  c.innerHTML = `
    <h2>🔓 Le coffre est ouvert !</h2>
    <div id="fin-dialogue"></div>
    ${serruresHTML()}
    <div class="article-secret">
      <div class="sceau">🏛️</div>
      <div class="numero">CONSTITUTION DU 4 OCTOBRE 1958 — ARTICLE 1er</div>
      <div class="texte">« La France est une République indivisible, laïque, démocratique et sociale.
        Elle assure l'égalité devant la loi de tous les citoyens sans distinction d'origine, de race ou de religion.
        Elle respecte toutes les croyances. »</div>
      <div style="font-size:.8rem;opacity:.75">Premier alinéa de l'article 1er, dans sa rédaction en vigueur.
        Source : Conseil constitutionnel, texte intégral de la Constitution.</div>
    </div>
    <p class="center" style="font-style:italic;opacity:.85">
      Les cinq mots des serrures forment la devise du coffre :<br>
      <b>« Les RÈGLES et les TEXTES donnent au PEUPLE la LOI — et son GARDIEN. »</b></p>
    <div class="grille-badges">
      ${badgeHTML("vitesse","⏱️","Équipe rapide","une salle en moins de 6 minutes")}
      ${badgeHTML("citoyen","🗳️","Citoyen éclairé","salle 3 sans aucun indice")}
      ${badgeHTML("juriste","📜","Fin juriste","salle 4 sans aucun indice")}
      ${badgeHTML("gardien","⚖️","Gardien de la Constitution","3 indices au maximum sur toute la partie")}
    </div>
...
const QUIZZ_FALLBACK = {
  CM1:[
    {q:"En quelle année la Constitution de la Ve République a-t-elle été promulguée ?", options:["1789","1958","2004"], bonne:1},
    {q:"Qui vote les lois ?", options:["Le Parlement","Le Président seul","Les juges"], bonne:0},
    {q:"Combien de membres compte le Conseil constitutionnel ?", options:["5","9","15"], bonne:1},
    {q:"Quelle est la devise de la République française ?", options:["Liberté, Égalité, Fraternité","Travail, Famille, Patrie","Unité, Force, Justice"], bonne:0},
    {q:"À partir de quel âge peut-on voter en France ?", options:["16 ans","18 ans","21 ans"], bonne:1}
  ],
  CM2:[
    {q:"Quels textes forment le « bloc de constitutionnalité » avec la Constitution de 1958 ?",
     options:["La DDHC de 1789, le Préambule de 1946 et la Charte de l'environnement","Toutes les lois votées depuis 1958","Les traités signés par la France"], bonne:0},
    {q:"Combien de temps le président de la République a-t-il pour promulguer une loi ?", options:["48 heures","15 jours","3 mois"], bonne:1},
    {q:"Que se passe-t-il si l'Assemblée nationale et le Sénat ne s'entendent pas ?",
     options:["La loi est abandonnée","Le Gouvernement peut demander à l'Assemblée nationale de statuer définitivement","Le Président tranche seul"], bonne:1},
    {q:"Qui nomme les neuf membres du Conseil constitutionnel ?",
     options:["Le président de la République, le président de l'Assemblée nationale et le président du Sénat","Les Français, par référendum","Le Premier ministre"], bonne:0},
    {q:"Selon l'article 1er, la France est une République…", options:["royale et militaire","indivisible, laïque, démocratique et sociale","fédérale et régionale"], bonne:1}
  ]
};
...
function afficherRecap(){
  ...
  const mention = part>=0.9 ? "🏆 Gardien de la Constitution"
                : part>=0.75 ? "🥈 Citoyen éclairé"
                : part>=0.55 ? "🥉 Apprenti juriste"
                : "📜 Apprenti motivé";
  ...
  recap.innerHTML = `
    <div class="article-secret">
      ...
      <div>🗝️ Serrures : <b>${ETAT.motsCles.length}/${NB_SALLES}</b></div>
